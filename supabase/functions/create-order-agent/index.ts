...
Deno.serve(async (req) => {
  ...
  try {
              ...
              items: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    unit_price: { type: "number", description: "Prix unitaire en euros (ex: 12.50)" },
                  },
    ...
    if (orderConfig.transfer_enabled) {
      ...
      console.log(`[CreateOrderAgent] Added webhook transfer tool`);
    }

    console.log(`[CreateOrderAgent] Total tools: ${tools.length}, names: ${tools.map(t => (t as Record<string, unknown>).name).join(", ")}`);
    ...
    console.log(`[CreateOrderAgent] Creating agent on ElevenLabs...`);
    ...
    if (!res.ok) {
      ...
      console.log(`[CreateOrderAgent] ElevenLabs error: ${res.status} — ${errorText}`);
    ...
    console.log(`[CreateOrderAgent] ElevenLabs agent created: ${data.agent_id}`);
    ...
    if (agentError) {
      console.log(`[CreateOrderAgent] DB error: ${agentError.message}`);
    ...
    // Save order config
    await supabase.from("agent_order_config").insert({
    ...
    console.log(`[CreateOrderAgent] Error: ${(error as Error).message}`);
