...
`.trim();
    ...
    // Add system tools directly to the tools array (same format as ElevenLabs UI)
    tools.push({
      type: "system",
      name: "end_call",
    ...
    // Build transfer webhook tool (uses Twilio API to redirect the call)
    console.log(`[CreateRdvAgent] transfer_enabled: ${rdvConfig.transfer_enabled}`);
    console.log(`[CreateRdvAgent] always_transfer: ${rdvConfig.always_transfer}`);
    console.log(`[CreateRdvAgent] default_transfer_number: ${rdvConfig.default_transfer_number}`);
    console.log(`[CreateRdvAgent] transfer_conditions: ${JSON.stringify(rdvConfig.transfer_conditions)}`);

    if (rdvConfig.transfer_enabled) {
      // Use a webhook tool instead of system tool (system tool doesn't work with register-call)
      tools.push({
        type: "webhook",
      ...
      console.log(`[CreateRdvAgent] Added webhook transfer tool "transferer_appel"`);
    }

    console.log(`[CreateRdvAgent] Total tools: ${tools.length}, names: ${tools.map(t => (t as Record<string,unknown>).name).join(", ")}`);
    ...
    const systemTools = tools.filter(t => (t as Record<string,unknown>).type === "system");
    console.log(`[CreateRdvAgent] Sending to ElevenLabs — ${tools.length} tools (${systemTools.length} system tools)`);
    console.log(`[CreateRdvAgent] System tools: ${JSON.stringify(systemTools, null, 2)}`);
    ...
    console.log(`[CreateRdvAgent] ElevenLabs response: agent_id=${data.agent_id}`);
    ...
    // Save RDV config
    await supabase.from("agent_rdv_config").insert({
