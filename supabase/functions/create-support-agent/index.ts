...
- Demande le numero de telephone${emailEnabled ? " et l'adresse email" : ""} du client si tu ne les as pas encore`.trim();
  ...
  let transferPrompt = "";
  if (transferEnabled) {
    // Build condition descriptions with phone numbers
...
Deno.serve(async (req) => {
  ...
  try {
        ...
        api_schema: {
          ...
          request_body_schema: {
            ...
            properties: {
              ...
              company: { type: "string", description: "Nom de l'entreprise du client (optionnel)" },
            },
      ...
      },
      {
        ...
        api_schema: {
          ...
          request_body_schema: {
            type: "object",
            properties: {
              action: { type: "string", description: "Toujours 'create_ticket'" },
              subject: { type: "string", description: "Sujet court du probleme (ex: 'Panne machine a cafe')" },
              description: { type: "string", description: "Description detaillee du probleme, incluant les etapes deja tentees" },
              priority: { type: "string", description: "Priorite: 'low', 'medium', 'high' ou 'urgent'" },
              category: { type: "string", description: "Categorie: 'general', 'technical', 'billing', 'feature_request' ou 'bug'" },
              client_phone: { type: "string", description: "Numero de telephone du client pour lier au contact" },
            },
            required: ["action", "subject", "description", "priority", "category"],
          },
        },
      },
      {
        ...
        api_schema: {
          ...
          request_body_schema: {
            type: "object",
            properties: {
              action: { type: "string", description: "Toujours 'update_ticket_status'" },
              case_number: { type: "string", description: "Le numero du ticket SAV (format SAV-XXXXXXXX-XXXXX)" },
              new_status: { type: "string", description: "Le nouveau statut: 'open', 'in_progress', 'waiting', 'resolved' ou 'closed'" },
            },
            required: ["action", "case_number", "new_status"],
          },
        },
      },
      {
        ...
        api_schema: {
          ...
          request_body_schema: {
            type: "object",
            properties: {
              action: { type: "string", description: "Toujours 'add_ticket_note'" },
              case_number: { type: "string", description: "Le numero du ticket SAV (format SAV-XXXXXXXX-XXXXX)" },
              content: { type: "string", description: "Le contenu de la note a ajouter" },
            },
            required: ["action", "case_number", "content"],
          },
        },
      },
      {
        ...
        api_schema: {
          ...
          request_body_schema: {
            ...
            properties: {
              ...
              phone_number: { type: "string", description: "Numero de telephone du destinataire au format international" },
              message: { type: "string", description: "Le contenu du SMS a envoyer" },
            },
            required: ["action", "phone_number", "message"],
          },
        },
      },
      {
        ...
        api_schema: {
          ...
          request_body_schema: {
            ...
            required: ["action", "email", "subject", "body"],
          },
        },
      },
      {
        ...
        api_schema: {
          ...
          request_body_schema: {
            type: "object",
            properties: {
              action: { type: "string", description: "Toujours 'schedule_meeting'" },
              client_name: { type: "string", description: "Nom complet du client" },
              client_phone: { type: "string", description: "Numero de telephone du client" },
              ...
              motif: { type: "string", description: "Motif du rendez-vous (ex: 'Intervention technique', 'Rappel client')" },
            },
            required: ["action", "client_name", "client_phone", "date", "time", "motif"],
          },
        },
      },
    ];
    ...
    if (supportConfig.transfer_enabled) {
      tools.push({
        type: "webhook",
      ...
      console.log(`[CreateSupportAgent] Added webhook transfer tool`);
    }

    console.log(`[CreateSupportAgent] Total tools: ${tools.length}, names: ${tools.map(t => (t as Record<string, unknown>).name).join(", ")}`);
    ...
    console.log(`[CreateSupportAgent] Creating agent on ElevenLabs...`);
    ...
    if (!res.ok) {
      ...
      console.log(`[CreateSupportAgent] ElevenLabs error: ${res.status} — ${errorText}`);
    ...
    console.log(`[CreateSupportAgent] ElevenLabs agent created: ${data.agent_id}`);
    ...
    if (agentError) {
      console.log(`[CreateSupportAgent] DB error: ${agentError.message}`);
    ...
    // Save support config
    await supabase.from("agent_support_config").insert({
    ...
    console.log(`[CreateSupportAgent] Config saved, agent_id: ${agentRecord.id}`);
    ...
    console.log(`[CreateSupportAgent] Error: ${(error as Error).message}`);
