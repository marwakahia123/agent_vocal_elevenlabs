...
import CommercialAgentForm from "./CommercialAgentForm";
...
export default function CreateCommercialAgentModal({ onClose, onCreated, editMode, agentId, initialData }: Props) {
  // Escape key handler
  useEffect(() => {
          ...
          <div className="flex items-start justify-between p-6 border-b border-slate-200">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 m-0">
                {editMode ? "Modifier - Agent Commercial" : "Configuration - Agent Commercial"}
              </h2>
              <p className="text-sm text-slate-500 mt-1 m-0">
                {editMode ? "Modifiez la configuration de votre agent de prospection commerciale" : "Configurez votre agent de prospection commerciale automatisee"}
              </p>
            </div>
