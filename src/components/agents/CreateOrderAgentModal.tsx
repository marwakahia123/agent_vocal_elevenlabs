...
import OrderAgentForm from "./OrderAgentForm";
...
export default function CreateOrderAgentModal({ onClose, onCreated, editMode, agentId, initialData }: Props) {
  // Escape key handler
  useEffect(() => {
          ...
          <div className="flex items-start justify-between p-6 border-b border-slate-200">
            <div>
              <h2 className="text-lg font-semibold text-slate-900 m-0">
                {editMode ? "Modifier - Agent Prise de Commande" : "Configuration - Agent Prise de Commande"}
              </h2>
              <p className="text-sm text-slate-500 mt-1 m-0">
                {editMode ? "Modifiez la configuration de votre agent de prise de commande" : "Configurez votre agent de prise de commande et facturation"}
              </p>
            </div>
